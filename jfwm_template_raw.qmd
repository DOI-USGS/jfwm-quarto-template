...
<!-- this may be too obscure, but I find the LyX (lyx.org) GUI to be really useful to construct complicated latex equations becuase it has a pointy-clicky interface from which you can pull raw code. Proposed additional sentence would be: Additionally, free graphical user interfaces such as LyX (lyx.org) provide visual tools to help create complicated equations from which the source LaTeX code can be copied and posted into a Quarto document.-->
...
<!-- should we mention I also switch to LaTeX for the final stages for some journals? -->
For example when writing agency reports our journal articles, we may stop using Quarto at the final stages because it is easier than continuing to update Quarto documents.
