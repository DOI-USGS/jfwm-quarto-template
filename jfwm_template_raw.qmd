...
- [x] MNF add in section on `quart-utils`
- [x] MNF create figure 1 _thanks Althea!!_
...
In hydrology, lack of reproducibility has raised questions of the vailidity of the field as a whole [@Hutton_2016] and has motivated exploration of challenges from other fields [@Fienen_2016].
...
<!-- might be good to mention the ubiquity of markdown useage in git as well? -->
...
Although our examples tend focus on R, Quarto works with many languages including LaTeX (e.g., for embedding typeset equations), Python, and Julia (Figure 1).
...
Code can be embedded within "chunks" that are defined by their language (e.g., `r` or `python`). These can either be displayed or hidden in the final document.
...
Further modifications can be done to change the template by updating the Style definitions (e.g., "Header 1" or "Body Text") in the template document. These changes would then be shown on the output Quarto-based Word document after saving the template and rendering the `.qmd`.

## `quarto-utils`

The `quarto-utils` software is a Python package to assist Quarto authors in handling references. 
By leveraging the doi.org web interface, authors are able to place references in their documents by only providing a DOI (Digital Object Identifier) in place of a reference in the text.
...
The author identifies a reference in the same way they would call out an existing reference using `[]` delimiters, but rather than providing a key that references the `.bib` file (e.g. `[@Prince1999]`) they would use either `_doi:thedoinumber` or `\_doi:thedoinumber` in its place. The `quarto-utils` code searches for `_doi:` or `\_doi:` and parses for the DOI.
...
For example, many weekly, monthly, quarterly, annually, or similar interval reports would benefit from not only using a computer script to run the statistical analysis or other number crunching, but also in the document preparation itself.
